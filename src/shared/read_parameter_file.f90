...
! local variables
  ...
  if (ierr /= 0) GPU_RUNTIME = 0 !stop 'an error occurred while reading the parameter file: GPU_RUNTIME'
  call read_value_string(GPU_PLATFORM, 'solver.GPU_PLATFORM', ierr)
  if (ierr /= 0) GPU_PLATFORM = "" !stop 'an error occurred while reading the parameter file: GPU_PLATFORM'
  call read_value_string(GPU_DEVICE, 'solver.GPU_DEVICE', ierr)
  if (ierr /= 0) GPU_DEVICE = "" !stop 'an error occurred while reading the parameter file: GPU_DEVICE'
