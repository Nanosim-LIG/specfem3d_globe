...
#include "prepare_constants_gpu.h"

#ifdef USE_OPENCL
cl_mem d_displ_cm_tex = NULL;
cl_mem d_accel_cm_tex = NULL;

cl_mem d_displ_oc_tex = NULL;
cl_mem d_accel_oc_tex = NULL;

cl_mem d_displ_ic_tex = NULL;
cl_mem d_accel_ic_tex = NULL;

cl_mem d_hprime_xx_cm_tex = NULL;
cl_mem d_hprime_xx_oc_tex = NULL;
cl_mem d_hprime_xx_ic_tex = NULL;
...
void moclEnqueueFillBuffer (Mesh *mp, cl_mem *buffer, int val, size_t size) {
  ...
  clCheck (clSetKernelArg (*memset_kern, 0, sizeof (cl_mem), (void *) buffer));
  clCheck (clSetKernelArg (*memset_kern, 0, sizeof (cl_int), (void *) &val));
...
void initialize_dummy_buffers(Mesh *mp) {
  ...
  static cl_mem dummy_rw_buffer ;
  cl_int errcode;

  dummy_rw_buffer = clCreateBuffer (mocl.context, CL_MEM_READ_WRITE, 10, NULL, clck_(&errcode));

#define INIT_DUMMY_BUFFER(field) mp->field.ocl = dummy_rw_buffer; clCheck(clRetainMemObject(dummy_rw_buffer));
...
void FC_FUNC_ (prepare_constants_device,
  ...
  *Mesh_pointer = (long) mp;
  ...
  // checks if NGLLX == 5
  if (*h_NGLLX != NGLLX) {
    exit_on_error ("NGLLX must be 5 for COL devices");
  }
  ...
  if (run_opencl) {
    ...
    d_hprime_xx_oc_tex = clCreateImage2D (mocl.context, CL_MEM_READ_ONLY,
                                          &format, NGLL2, 1, 0, mp->d_hprime_xx,
                                          clck_(&errcode));
    d_hprime_xx_ic_tex = clCreateImage2D (mocl.context, CL_MEM_READ_ONLY,
                                          &format, NGLL2, 1, 0, mp->d_hprime_xx,
                                          clck_(&errcode));
    d_hprime_xx_cm_tex = clCreateImage2D (mocl.context, CL_MEM_READ_ONLY,
                                          &format, NGLL2, 1, 0, mp->d_hprime_xx,
                                          clck_(&errcode));
  ...
  if (mp->nadj_rec_local > 0) {
    ...
    mp->h_adj_sourcearrays_slice =
      (realw *) malloc (mp->nadj_rec_local * NDIM * NGLL3 * sizeof (realw));
...
void FC_FUNC_ (prepare_fields_rotation_device,
  ...
  if (!mp->rotation) {
    exit_on_gpu_error ("prepare_fields_rotation_device: rotation flag not properly initialized");
  }
...
#ifdef USE_CUDA
    if (run_cuda) {
      print_CUDA_error_if_any(cudaMalloc((void**)&(mp->d_send_accel_buffer_crust_mantle.cuda),
...
#ifdef USE_CUDA
      if (run_cuda) {
        print_CUDA_error_if_any(cudaMalloc((void**)&(mp->d_b_send_accel_buffer_crust_mantle.cuda),
                                           NDIM*(mp->max_nibool_interfaces_cm)*(mp->num_interfaces_crust_mantle)*sizeof(realw)),4004);
      }
#endif
    }
...
void FC_FUNC_ (prepare_fields_noise_device,
  ...
  if (mp->nspec2D_top_crust_mantle > 0) {
    ...
    exit_on_gpu_error ("prepare_fields_noise_device NSPEC_TOP not properly initialized");
  }
...
void FC_FUNC_ (prepare_oceans_device,
  ...
  if (mp->npoin_oceans == 0) {
    exit_on_gpu_error ("prepare_oceans_device has zero npoin_oceans");
  }
...
void FC_FUNC_ (prepare_crust_mantle_device,
  ...
  if (run_opencl) {
    ...
    d_displ_cm_tex = clCreateImage2D (mocl.context, CL_MEM_READ_ONLY,
                                      &format, size, 1, 0, mp->d_displ_crust_mantle.ocl
                                      clck_(&errcode));
    d_accel_cm_tex = clCreateImage2D (mocl.context, CL_MEM_READ_ONLY,
                                      &format, size, 1, 0, mp->d_accel_crust_mantle.ocl,
                                      clck_(&errcode));
#endif
  }
#endif
  ...
  if (run_cuda) {
#ifdef USE_TEXTURES_FIELDS
    {
      ...
      cudaChannelFormatDesc channelDesc1 = cudaCreateChannelDesc<realw>();
      print_CUDA_error_if_any(cudaBindTexture(0, mp->d_displ_cm_tex_ref_ptr, mp->d_displ_crust_mantle.cuda,
...
void FC_FUNC_ (prepare_outer_core_device,
...
#ifdef USE_TEXTURES_FIELDS
    cl_image_format format = {CL_R, CL_UNSIGNED_INT32};
    d_displ_oc_tex = clCreateImage2D (mocl.context, CL_MEM_READ_ONLY,
                                      &format, size_glob, 1, 0,
                                      mp->d_displ_outer_core.ocl,
                                      clck_(&errcode));
    d_accel_oc_tex = clCreateImage2D (mocl.context, CL_MEM_READ_ONLY,
                                      &format, size_glob, 1, 0,
                                      mp->d_accel_outer_core.ocl,
                                      clck_(&errcode));
#endif
  }
#endif
  ...
  if (run_cuda) {
      ...
      cudaChannelFormatDesc channelDesc1 = cudaCreateChannelDesc<realw>();
      print_CUDA_error_if_any(cudaBindTexture(0, mp->d_displ_oc_tex_ref_ptr, mp->d_displ_outer_core.cuda,
...
#ifdef USE_TEXTURES_FIELDS
    cl_image_format format = {CL_R, CL_UNSIGNED_INT32};
    d_displ_ic_tex = clCreateImage2D (mocl.context, CL_MEM_READ_ONLY,
                                      &format, size, 1, 0, mp->d_displ_inner_core,
                                      clck_(&errcode));
    d_accel_ic_tex = clCreateImage2D (mocl.context, CL_MEM_READ_ONLY,
                                      &format, size, 1, 0, mp->d_accel_inner_core,
                                      clck_(&errcode));
#endif
  }
#endif
...
#ifdef USE_TEXTURES_FIELDS
    clReleaseMemObject (d_displ_cm_tex);
    clReleaseMemObject (d_accel_cm_tex);

    clReleaseMemObject (d_displ_oc_tex);
    clReleaseMemObject (d_accel_oc_tex);

    clReleaseMemObject (d_displ_ic_tex);
    clReleaseMemObject (d_accel_ic_tex);
#endif
#ifdef USE_TEXTURES_CONSTANTS
    clReleaseMemObject (d_hprime_xx_cm_tex);
    clReleaseMemObject (d_hprime_xx_oc_tex);
    clReleaseMemObject (d_hprime_xx_ic_tex);
#endif
