...
void FC_FUNC_ (compute_stacey_acoustic_gpu,
  ...
  if (run_opencl) {
    ...
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 0, sizeof (gpu_realw_mem), (void *) &mp->d_veloc_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 1, sizeof (gpu_realw_mem), (void *) &mp->d_accel_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 2, sizeof (int), (void *) &interface_type));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 3, sizeof (int), (void *) &num_abs_boundary_faces));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 4, sizeof (gpu_int_mem), (void *) &d_abs_boundary_ispec->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 5, sizeof (gpu_int_mem), (void *) &mp->d_nkmin_xi_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 6, sizeof (gpu_int_mem), (void *) &mp->d_nkmin_eta_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 7, sizeof (gpu_int_mem), (void *) &mp->d_njmin_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 8, sizeof (gpu_int_mem), (void *) &mp->d_njmax_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 9, sizeof (gpu_int_mem), (void *) &mp->d_nimin_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 10, sizeof (gpu_int_mem), (void *) &mp->d_nimax_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 11, sizeof (gpu_realw_mem), (void *) &d_abs_boundary_jacobian2D->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 12, sizeof (gpu_realw_mem), (void *) &d_wgllwgll->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 13, sizeof (gpu_int_mem), (void *) &mp->d_ibool_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 14, sizeof (gpu_realw_mem), (void *) &mp->d_vp_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 15, sizeof (int), (void *) &mp->save_forward));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 16, sizeof (gpu_realw_mem), (void *) &d_b_absorb_potential->ocl));
...
void FC_FUNC_ (compute_stacey_acoustic_backward_gpu,
  ...
  if (run_opencl) {
    ...
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 0, sizeof (gpu_realw_mem), (void *) &mp->d_b_accel_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 1, sizeof (gpu_int_mem), (void *) &d_b_absorb_potential->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 2, sizeof (int), (void *) &interface_type));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 3, sizeof (int), (void *) &num_abs_boundary_faces));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 4, sizeof (gpu_realw_mem), (void *) &d_abs_boundary_ispec->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 5, sizeof (gpu_int_mem), (void *) &mp->d_nkmin_xi_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 6, sizeof (gpu_int_mem), (void *) &mp->d_nkmin_eta_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 7, sizeof (gpu_int_mem), (void *) &mp->d_njmin_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 8, sizeof (gpu_int_mem), (void *) &mp->d_njmax_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 9, sizeof (gpu_int_mem), (void *) &mp->d_nimin_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 10, sizeof (gpu_int_mem), (void *) &mp->d_nimax_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_backward_kernel, 11, sizeof (gpu_int_mem), (void *) &mp->d_ibool_outer_core.ocl));
...
void FC_FUNC_ (compute_stacey_acoustic_undoatt_gpu,
  ...
  if (run_opencl) {
    ...
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 0, sizeof (gpu_realw_mem), (void *) &mp->d_b_veloc_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 1, sizeof (gpu_realw_mem), (void *) &mp->d_b_accel_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 2, sizeof (int), (void *) &interface_type));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 3, sizeof (int), (void *) &num_abs_boundary_faces));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 4, sizeof (gpu_int_mem), (void *) &d_abs_boundary_ispec->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 5, sizeof (gpu_int_mem), (void *) &mp->d_nkmin_xi_outer_core.ocl));
    ...
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 14, sizeof (gpu_realw_mem), (void *) &mp->d_vp_outer_core.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 15, sizeof (int), (void *) &mp->save_forward));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_acoustic_kernel, 16, sizeof (gpu_int_mem), (void *) &d_b_absorb_potential->ocl));
