...
void FC_FUNC_ (compute_stacey_elastic_gpu,
  ...
  if (run_opencl) {
    ...
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 0, sizeof (gpu_realw_mem), (void *) &mp->d_veloc_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 1, sizeof (gpu_realw_mem), (void *) &mp->d_accel_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 2, sizeof (int), (void *) &interface_type));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 3, sizeof (int), (void *) &num_abs_boundary_faces));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 4, sizeof (gpu_int_mem), (void *) &d_abs_boundary_ispec->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 5, sizeof (gpu_int_mem), (void *) &mp->d_nkmin_xi_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 6, sizeof (gpu_int_mem), (void *) &mp->d_nkmin_eta_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 7, sizeof (gpu_int_mem), (void *) &mp->d_njmin_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 8, sizeof (gpu_int_mem), (void *) &mp->d_njmax_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 9, sizeof (gpu_int_mem), (void *) &mp->d_nimin_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 10, sizeof (gpu_int_mem), (void *) &mp->d_nimax_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 11, sizeof (gpu_realw_mem), (void *) &d_abs_boundary_normal->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 12, sizeof (gpu_realw_mem), (void *) &d_abs_boundary_jacobian2D->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 13, sizeof (gpu_realw_mem), (void *) &d_wgllwgll->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 14, sizeof (gpu_int_mem), (void *) &mp->d_ibool_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 15, sizeof (gpu_realw_mem), (void *) &mp->d_rho_vp_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 16, sizeof (gpu_realw_mem), (void *) &mp->d_rho_vs_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 17, sizeof (int), (void *) &mp->save_forward));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 18, sizeof (gpu_realw_mem), (void *) &d_b_absorb_field->ocl));
...
void FC_FUNC_ (compute_stacey_elastic_backward_gpu,
  ...
  if (run_opencl) {
    ...
    // absorbing boundary contributions
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 0, sizeof (gpu_realw_mem), (void *) &mp->d_b_accel_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 1, sizeof (gpu_realw_mem), (void *) &d_b_absorb_field->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 2, sizeof (int), (void *) &interface_type));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 3, sizeof (int), (void *) &num_abs_boundary_faces));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 4, sizeof (gpu_int_mem), (void *) &d_abs_boundary_ispec->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 5, sizeof (gpu_int_mem), (void *) &mp->d_nkmin_xi_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 6, sizeof (gpu_int_mem), (void *) &mp->d_nkmin_eta_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 7, sizeof (gpu_int_mem), (void *) &mp->d_njmin_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 8, sizeof (gpu_int_mem), (void *) &mp->d_njmax_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 9, sizeof (gpu_int_mem), (void *) &mp->d_nimin_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 10, sizeof (gpu_int_mem), (void *) &mp->d_nimax_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_backward_kernel, 11, sizeof (gpu_int_mem), (void *) &mp->d_ibool_crust_mantle.ocl));
...
void FC_FUNC_ (compute_stacey_elastic_undoatt_gpu,
  ...
  if (run_opencl) {
    ...
    // absorbing boundary contributions
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 0, sizeof (gpu_realw_mem), (void *) &mp->d_b_veloc_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 1, sizeof (gpu_realw_mem), (void *) &mp->d_b_accel_crust_mantle.ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 2, sizeof (int), (void *) &interface_type));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 3, sizeof (int), (void *) &num_abs_boundary_faces));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 4, sizeof (gpu_int_mem), (void *) &d_abs_boundary_ispec->ocl));
    clCheck (clSetKernelArg (mocl.kernels.compute_stacey_elastic_kernel, 5, sizeof (gpu_int_mem), (void *) &mp->d_nkmin_xi_crust_mantle.ocl));
