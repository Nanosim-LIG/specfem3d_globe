...
for imp in to_import:
    ...
    globals()[name] = sh.Command(imp) if not DEBUG else bake_echo(name)
...
def analyze_results(scenarii):
    ...
    return
    to_plot = []
    seismo_glob = "specfem-{}/OUTPUT_FILES/*.ascii".format(VERSIONS_TO_TEST[0])
...
class Scenario:
    ...
    def initialize_build_dir(self):
        ...
        cd(BUILD_DIR)
        mkdir("specfem-{}".format(self.runtime), "-p")
        cd("specfem-{}".format(self.runtime))
    ...
    def setup_environment(self):
        ...
        print("build the example")
        print(make("CFLAGS='{}'".format(" ".join(["-D{}".format(flag) for flag in self.make_flags]))))
        print(make("build-example"))
